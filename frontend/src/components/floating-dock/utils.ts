 ...
 */
export const determineDockSide = (
  centerX: number,
  parentWidth: number,
  options?: { allowOutsideLeft?: boolean },
): DockSide => {
  const allowOutsideLeft = options?.allowOutsideLeft ?? true;
  if (centerX < 0) {
    return allowOutsideLeft ? 'outside-left' : 'inside-left';
  }
