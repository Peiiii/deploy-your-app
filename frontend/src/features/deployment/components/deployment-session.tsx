...
export const DeploymentSession: React.FC<DeploymentSessionProps> = ({
  ...
  if (state.deploymentStatus === DeploymentStatus.ANALYZING) {
    return (
      <div className={`bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow ${className || ''}`}>
  ...
  if (state.deploymentStatus === DeploymentStatus.SUCCESS) {
    if (!resolvedProject && !projectUrlOverride) {
      ...
      if (projectsLoading) {
        return (
          <div className={`bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-8 text-center shadow ${className || ''}`}>
      ...
      return (
        <div className={`bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow space-y-4 ${className || ''}`}>
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-green-200 dark:border-green-900/50 shadow-lg overflow-hidden">
          ...
          {/* Connected Build Log Section */}
          {state.logs.length > 0 && (
            <div className="bg-white dark:bg-slate-900 border-t border-green-200 dark:border-green-900/50">
              <button
  ...
  if (isInProgress) {
    ...
    return (
      <div className={`bg-slate-900 dark:bg-black rounded-xl border border-slate-800 dark:border-slate-900 overflow-hidden shadow-xl ${className || ''}`}>
