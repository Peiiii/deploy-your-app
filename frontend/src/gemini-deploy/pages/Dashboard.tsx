    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <button
            onClick={() => requireAuthAnd(() => navigate('/deploy'))}
...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, analytics, onNavigate, onCopyUrl, copiedUrl, t }) => {
  ...
  return (
    ...
    >
      {/* Status Indicator */}
      <div className={`absolute top-4 right-4 flex items-center gap-2 px-2.5 py-1 rounded-full text-[10px] font-bold border uppercase tracking-wider z-10 ${project.status === 'Live' ? 'bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/20' :
          project.status === 'Failed' ? 'bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/20' :
            'bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 border-yellow-500/20'
        }`}>
        <div className={`w-1.5 h-1.5 rounded-full ${project.status === 'Live' ? 'bg-green-500 dark:bg-green-400 animate-pulse' :
      ...
      <div className="flex items-start gap-4 mb-5">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center border shadow-inner flex-shrink-0 transition-transform group-hover:scale-110 ${project.framework === 'React'
