...
The alternatives were a fixed delay, which would only hide the race, and sidebar-only request flags, which would duplicate shared data state. Shared lifecycle state is the smallest durable fix and can be reused by other project consumers.
