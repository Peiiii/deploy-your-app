...
import { collect, reserve, dayKey } from '../packages/product-analytics/src/budget';
...
try {
  ...
  for (const file of readdirSync('packages/product-analytics/migrations').sort()) {
    for (const sql of readFileSync(`packages/product-analytics/migrations/${file}`, 'utf8').split(';').filter(s => s.trim())) await db.prepare(sql).run();
  }
  ...
  const batch: EventBatch = { visitorId: crypto.randomUUID(), sessionId: crypto.randomUUID(), device: 'desktop', referrer: 'direct', channel: 'web', events: [{ id: crypto.randomUUID(), name: 'page_view', at: Date.now(), page: 'home' }] };
  ...
  const before = (await db.prepare('SELECT COUNT(*) n FROM product_event_limits').first()).n;
  await collect(db, { ...batch, visitorId: crypto.randomUUID() }, { signedIn: false, admin: false }, 'standalone');
  assert.equal((await db.prepare('SELECT COUNT(*) n FROM product_event_limits').first()).n, before, 'site exhaustion must not create new per-visitor quota rows');
